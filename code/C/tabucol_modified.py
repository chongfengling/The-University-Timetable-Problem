...
if __name__ == "__main__":
    sols = []
    # for ntest in [5,50,100,200,500]:
    for ntest in range(50,100):
        print('number of nodes: ', ntest)
        # chromatic number of mycielski_graph(n) is n
        G = nx.gnp_random_graph(ntest,0.5)
        colors = {}
        ...
        with open('TABUCOL_sol.txt', 'a') as f:
            f.write('\n'+ str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))+'\n')
            f.write('number of nodes: ' + str(ntest) +'\n')
            f.write(str(sol) + '\n')
        del G
    print("\n")
    for sol in sols:
